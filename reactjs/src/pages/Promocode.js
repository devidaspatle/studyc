...
function Promocode() {
    ...
    const handleSave = () => {
        setIsSaving(true);
        axios.post('http://localhost:4000/api/promocodes', {
            promocode: promocode,
            price: price
          })
   ...
   useEffect(() => {
    axios.get(`http://localhost:4000/api/promocodes/${id}`)
    .then(function (response) {
        let promocodes = response.data
        setId(promocodes.id);
        setPromocode(promocodes.promocode);
        setPrice(promocodes.price);
   ...
   const fetchPromocodeList = () => {
       axios.get('http://localhost:4000/api/promocodes')
       .then(function (response) {
   ...
   const handleDelete = (id) => {
       Swal.fire({
           ...
           if (result.isConfirmed) {
               axios.delete(`http://localhost:4000/api/promocodes/${id}`)
               .then(function (response) {
                   Swal.fire({
                       icon: 'success',
